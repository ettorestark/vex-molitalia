<div class="wrap">
	...
	$objDaoComprobante = new vexfe_dao_comprobante_factura();
  ...
  list($rsFacturaEnvio) = $objDaoEnvio->get(array('comprobante_id' => $comprobante_id,
	...
	<div>
		...
		<?PHP
			...
			if(  intval($comprobante['sunat_envios_conformes']) > 0 )
			{

				$enviado = true;
				echo '<button class="button action btn_factura_estado btn_estado_ok" data-comprobante-id="'.$comprobante['id'].' "> <i class="fa fa-check"></i> <span> Enviado </span> </button> ';

			}
			else if( intval($comprobante['sunat_envios_fallidos']) > 0 )
			{

				echo '<button class="button action btn_factura_estado btn_estado_reenviar" data-comprobante-id="'.$comprobante['id'].' "> <i class="fa fa-times"></i> <span> Reenviar ('.$comprobante['sunat_envios_fallidos'].') </span>  </button> ';
			}
			else {

				echo '<button class="button action btn_factura_estado btn_estado_pendiente" data-comprobante-id="'.$comprobante['id'].' "> <i class="fa fa-paper-plane"></i>   <span> Pendiente </span>  </button> ';
